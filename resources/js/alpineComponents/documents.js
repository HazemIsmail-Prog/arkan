document.addEventListener('alpine:init', () => {
    Alpine.data('documentsComponent', () => ({
        ...
        getLastUpdatedAttachment(document) {
            if(document.attachments && document.attachments.length > 0) {
                let max = document.attachments.reduce((max, attachment) => {
                    return Math.max(max, new Date(attachment.updated_at).getTime());
        ...
        handleAttachmentAddedEvent(event) {
            ...
            if(incomingAttachableType === 'document') {
                let index = this.documents.findIndex(document => document.id === attachableId);
                if(index !== -1) {
                    if(this.documents[index].attachments) {
                        this.documents[index].attachments.push(incomingAttachment);
                    } else {
                        this.documents[index].attachments = [incomingAttachment];
                    }
                }
            }
        },
